...
import random
from EVSystemV2_0 import EVSystem
...
class MyEnvironment(gym.Env):
    ...
    def step(self, action):
        # 在这里实现环境的状态转移逻辑
        self.action = action
        if self.request != [0, 0, 0, 0]:
            self.reward = self.EVSystem.EVstations[action].add_task(self.request)
        if self.renew_flag:
            self.EVSystem.renew_state()
        self.request, self.renew_flag = self.EVSystem.creat_task()

        state = OrderedDict()
        state["stask"] = np.array(self.request[0:2])
